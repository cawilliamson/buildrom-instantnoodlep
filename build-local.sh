...
docker run --user $(id -u):$(id -g) --privileged --tty --rm -e DEVICE="${1}" -e ROM="${2}" -v "$(pwd)/src:/usr/src/rom" -v "$(pwd):/var/tmp/buildrom" -v "$(pwd)/tmp/${1}/${2}:/var/tmp/buildrom/out" chrisawcom/buildrom
